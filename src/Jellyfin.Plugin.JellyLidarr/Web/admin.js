(function () {
  ...
  function fillOptions(id, items, selected) {
    if (!Array.isArray(items)) throw new Error('Invalid options response. Check that JellyLidarr 1.0.0.10 is loaded after restarting Jellyfin.');
    const select = $(id);
  ...
  run(async()=>{
    ...
    if(typeof settings?.lidarrUrl!=='string') throw new Error('Invalid settings response. Restart Jellyfin to load JellyLidarr 1.0.0.10.');
    config=settings;
