...
http.createServer(async(req,res)=>{
  ...
  try{
    ...
    if(url.pathname.startsWith('/JellyLidarr/assets/')){
      const name=url.pathname.split('/').pop();if(!['admin.css','admin.js','style.css','app.js'].includes(name))throw new Error('Unknown asset');
      res.setHeader('Content-Type',name.endsWith('.css')?'text/css':'text/javascript');res.end(await readFile(new URL(name,web)));return;
    }
    ...
    if(url.pathname==='/JellyLidarr/me')return json({id:'admin',name:'Administrator',role:'Approver',isAdministrator:true});
